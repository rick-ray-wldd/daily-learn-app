 ...
 * ## 為什麼這不算違反「顏色帶語意」那條鐵律
 *
 * 那條規則管的是 `C` 裡的**語意色**：綠＝學習者動手了、琥珀＝app 在猜、
 * 藍＝中性 chrome。難度帶不是那三種語意的任何一種，它是**材質屬性**——
 * ADR-0018 已經把材質 token 與語意 token 分成兩套，這一組屬於材質那一套，
 * 跟 `GLASS` 是同層的東西。
 *
 * 但它必須**避開那三個色相**，否則語意色的辨識度會被稀釋。所以色階刻意走
 * 青綠→紫→洋紅→玫瑰（190° / 262° / 292° / 322° / 346°），與綠 142°、
 * 琥珀 43°、藍 217° 都拉開至少 45°。
 *
 * ## 為什麼 fill 很淡而 edge 較濃
 *
 * Apple Music 那種質感的來源是「顏色當重點，不是當色塊」。填色壓到 0.10 以下，
 * 讓封面本身還是視覺主角；等級的資訊量交給**邊框**去帶。整片色塊會蓋掉封面，
 * 而封面才是使用者真正在掃描的東西。
 *
 * ⚠️ 不要用紅色收尾。這個 app 還沒有 danger token，但遲早會有，先把 346° 留在
 * 玫瑰而不是正紅，之後加 danger 才不會撞。
 */
export const LEVEL: Record<1 | 2 | 3 | 4 | 5, { fill: string; edge: string; ink: string }> = {
  1: { fill: 'rgba(56, 189, 216, 0.09)',  edge: 'rgba(56, 189, 216, 0.30)',  ink: '#7FD8E8' },
  2: { fill: 'rgba(150, 122, 240, 0.09)', edge: 'rgba(150, 122, 240, 0.30)', ink: '#B4A2F5' },
  3: { fill: 'rgba(198, 112, 228, 0.09)', edge: 'rgba(198, 112, 228, 0.30)', ink: '#DCA4EE' },
  4: { fill: 'rgba(228, 104, 186, 0.10)', edge: 'rgba(228, 104, 186, 0.32)', ink: '#F0A0CE' },
  5: { fill: 'rgba(235, 92, 132, 0.10)',  edge: 'rgba(235, 92, 132, 0.34)',  ink: '#F79FB4' },
};

/** 沒有任何依據時的中性玻璃。**不要拿等級 3 當預設**——那是在猜。 */
export const LEVEL_UNKNOWN = {
  fill: GLASS.fill,
  edge: GLASS.edge,
  ink: C.faint,
};
